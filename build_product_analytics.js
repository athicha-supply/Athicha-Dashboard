...
for(const row of sales){
  ...
  const p=map.get(code)||{code,product:clean(row.stkdes)||code,value:0,qty:0,documents:new Set(),customers:new Set(),months:new Set(),lastMonth:0};
  p.value+=Number(row.Total)||0;p.qty+=Number(row.Qty)||0;p.documents.add(clean(row.docnum));p.customers.add(clean(row.cuscod));p.months.add(month);p.lastMonth=Math.max(p.lastMonth,month);map.set(code,p);
}
const result={};
for(const [year,map] of [...yearly.entries()].sort()){
  ...
  const products=[...map.values()].map(p=>{
    ...
    const growth=prev&&prev.value!==0?(p.value-prev.value)/Math.abs(prev.value)*100:null;
    ...
    return {code:p.code,product:p.product,value:p.value,qty:p.qty,documents:p.documents.size,customers:p.customers.size,activeMonths:p.months.size,lastMonth:p.lastMonth,avgPrice:p.qty?p.value/p.qty:0,previousValue:prev?.value||0,growth,stockQty:stk.stockQty,stockValue:stk.stockValue,turnover,signal};
